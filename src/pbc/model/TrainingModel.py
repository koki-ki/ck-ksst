...
class MaximumBayesBoundarynessTraining(TrainingModel):
    ...
    def build_model(self):
        ...
        H_hat = tf.where(tf.is_finite(H_hat), H_hat, tf.zeros_like(H_hat))

        # S * 1
        U_hat = tf.subtract(1.0, H_hat)

        # S * 1
        l = tf.multiply(self.W, U_hat)
        ...
        # S * 1
        diff_k_by_d = tf.multiply(tf.divide(tf.multiply(-1.0, self.dy_S), tf.square(self.h_S)), k)
        # S * c
        diff_k_by_g = tf.multiply(diff_dy_by_g, diff_k_by_d)

        # S * 1 (1)
        diff_W_by_kx = tf.multiply(U_hat, tf.divide(1.0, sum_k))
        # S * c -> S * c * 1
        diff_W_by_gx = tf.expand_dims(tf.multiply(diff_k_by_g, diff_W_by_kx), axis=2)
        # S * c * p -> S * c * p * d
        diff_W_by_gx_proto = tf.expand_dims(tf.multiply(nearest_proto_ind_S, diff_W_by_gx), axis=3)
        # S * c * p * d
        diff_W_by_Ax = tf.multiply(diff_W_by_gx_proto, diff_g_by_A)

        # S * 1 (S * 1)
        diff_W_by_ku = tf.multiply(U_hat, tf.divide(tf.multiply(-1.0, k), tf.square(sum_k)))

        # S * c -> S * c * 1
        diff_k_by_gu = tf.expand_dims(diff_k_by_g, axis=2)
        # S * c * p -> S * c * p * d
        diff_k_by_gu_proto = tf.expand_dims(tf.multiply(nearest_proto_ind_S, diff_k_by_gu), axis=3)
        # S * c * p * d
        diff_k_by_Au = tf.multiply(diff_k_by_gu_proto, diff_g_by_A)
        # c * p * d
        sum_diff_k_by_Au = tf.reduce_sum(diff_k_by_Au, axis=0)
        # S * c * p * d
        diff_W_by_Au = tf.map_fn(lambda x: tf.multiply(sum_diff_k_by_Au, x), diff_W_by_ku)

        # S * c * p * d
        first_term = tf.add(diff_W_by_Ax, diff_W_by_Au)
        ...
        # S * c * p * d
        diff_l_by_A = tf.add(first_term, second_term)
