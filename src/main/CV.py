...
def make_CV_index_list(x, y, cv_fold=160, seed=0):
    ...
    for k in range(cv_fold):
        ...
        for yc in y_cat:
            ...
            yc_ts_idx = np.random.choice(nu_ind, size=y_cat_N_k[yc], replace=False)
            cur_ts_list.extend(yc_ts_idx)
        used_ts_index.extend(cur_ts_list)

        cur_tr_list = list(set(all_index) - set(cur_ts_list))
        cur_tr_list = np.random.choice(cur_tr_list, size=len(cur_tr_list), replace=False)
        tr_index_list.append(cur_tr_list)

        cur_ts_list = np.random.choice(cur_ts_list, size=len(cur_ts_list), replace=False)
...
def CV_SVM(dataset_path, out_dir=None, cv_fold=160, svm_c=range(-15, 16), svm_gamma=range(-15, 16)):
    ...
    for sc in svm_c:
        ...
        for sg in svm_gamma:
            ...
            gamma = 2**sg
            Ltr, Lval = 0.0, 0.0
            train_idx_list, test_idx_list = make_CV_index_list(x, y, cv_fold=cv_fold)
            for tril, tsil in zip(train_idx_list, test_idx_list):
                ...
                y_pred_train, y_pred_test = cls.predict(x_train), cls.predict(x_test)
            ...
            print("CV_SVM %s - Etr:%.5f, Eval:%.5f (C:%s, gamma:%s)" % (dataset_name, Ltr, Lval, C, gamma))
    ...
    min_Tr, min_Tr_C, min_Tr_gamma = result_list[min_Tr_index, 2], result_list[min_Tr_index, 0], result_list[min_Tr_index, 1]
    min_Ts, min_Ts_C, min_Ts_gamma = result_list[min_Ts_index, 3], result_list[min_Ts_index, 0], result_list[min_Ts_index, 1]

    print("CV_SVM %s: Minimum Etr: %.5f (C:2**%s, gamma:2**%s), Minimum Eval: %.5f (C:2**%s, gamma:2**%s)" % (dataset_name, min_Tr, min_Tr_C, min_Tr_gamma, min_Ts, min_Ts_C, min_Ts_gamma))
    ...
    info.append(["CV_SVM: Minimum training error (C=%s gamma=%s)" % (min_Tr_C, min_Tr_gamma), min_Tr])
    info.append(["CV_SVM: Minimum validation error (C=%s gamma=%s)" % (min_Ts_C, min_Ts_gamma), min_Ts])
    ...
    extent = [seq_param_gamma[0], seq_param_gamma[-1], seq_param_C[-1], seq_param_C[0]]
    ...
    fig.suptitle("CV_SVN %s: Minimum Etr: %.5f (C:2**%s, gamma:2**%s)" % (dataset_name, min_Tr, min_Tr_C, min_Tr_gamma))
    ...
    fig.suptitle("CV_SVN %s: Minimum Eval: %.5f (C:2**%s, gamma:2**%s)" % (dataset_name, min_Ts, min_Ts_C, min_Ts_gamma))
    save_plt_fig(fig, os.path.join(image_dir, "%s_Eval.png" % res_basename))

    # All result
    save_txt(os.path.join(out_dir, "%s.csv" % res_basename), result_list, header=["SVM_C(2**)", "SVM_gamma(2**)", "Etr", "Eval"])
...
def CV_PBC(dataset_path, out_dir=None, cv_fold=160, pbc_p=range(1, 101)):
    ...
    for p in pbc_p:
        ...
        for tril, tsil in zip(train_idx_list, test_idx_list):
            ...
            proto = np.array([kmeans.fit(x_train[np.where(np.argmax(y_train, axis=1)==cls)]).cluster_centers_ for cls in range(class_num)])
            ...
            with tf.Session(graph=tf.get_default_graph(), config=session_conf) as sess:
                ...
                closed_error = sess.run(error, feed_dict={x_data: x_train, y_target: y_train})
                open_error = sess.run(error, feed_dict={x_data: x_test, y_target: y_test})
        ...
        print("CV_PBC %s - Etr:%.5f, Eval:%.5f (p:%s)" % (dataset_name, Ltr, Lval, p))
    ...
    min_Tr_p, min_Tr = result_list[min_Tr_index, 0], result_list[min_Tr_index, 1]
    min_Ts_p, min_Ts = result_list[min_Ts_index, 0], result_list[min_Ts_index, 2]

    print("CV_PBC %s: Minimum Etr: %.5f (p:%s), Minimum Eval: %.5f (p:%s)" % (dataset_name, min_Tr, min_Tr_p, min_Ts, min_Ts_p))
    ...
    info.append(["CV_PBC: Minimum validation error (p=%s)" % (min_Ts_p), min_Ts])
    info.append(["Runtime", format_sec_to_hours(float(end - start))])
    save_txt(os.path.join(out_dir, "log_%s.txt" % res_basename), info)
    ...
    ax.plot(result_list[:, 0], result_list[:, 1], c="red", linewidth=15, label="Training Error")
    ax.plot(result_list[:, 0], result_list[:, 2], c="orange", linewidth=15, label="Validating Error")
    ax.legend(loc="best")
    fig.suptitle("CV_PBC %s: Min Etr: %.5f (p:%s), Min Eval: %.5f (p:%s)" % (dataset_name, min_Tr, min_Tr_p, min_Ts, min_Ts_p))
    save_plt_fig(fig, os.path.join(image_dir, "%s_EtrEval.png" % res_basename))

    # All result
    save_txt(os.path.join(out_dir, "%s.csv" % res_basename), result_list, header=["PBC_p", "Etr", "Eval"])


if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog=SCRIPT_NAME)
    parser.add_argument("-d", "--dataset", help="The path of dataset (default: ../../dataset/GMM5C.csv)", type=str, default="../../dataset/GMM5C.csv")
    parser.add_argument("-o", "--output", help="The path to output directory (default: ../../res/CV", type=str, default="../../res/CV")
    ...
    fold = args.fold
    min_pbc_num = int(args.min_p[1:]) if args.min_p[0] == "p" else -int(args.min_p[1:])
    max_pbc_num = int(args.max_p[1:]) if args.max_p[0] == "p" else -int(args.max_p[1:])
    min_svm_num = int(args.min_s[1:]) if args.min_s[0] == "p" else -int(args.min_s[1:])
    max_svm_num = int(args.max_s[1:]) if args.max_s[0] == "p" else -int(args.max_s[1:])
    mode = args.mode
    ...
    if mode == "PBC" or mode == "ALL":
        CV_PBC(dataset_path, out_dir=output_directory, cv_fold=fold, pbc_p=range(min_pbc_num, max_pbc_num+1))
    if mode == "SVM" or mode == "ALL":
        CV_SVM(dataset_path, out_dir=output_directory, cv_fold=fold, svm_c=range(min_svm_num, max_svm_num+1), svm_gamma=range(min_svm_num, max_svm_num+1))
