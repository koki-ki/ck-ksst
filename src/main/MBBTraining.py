...
from lib.argument import get_MBBT_args
from lib.load_data import load_data
from pbc.model.ClassifierModel import MultiClassKmeansPrototypeClassifier
from pbc.model.OptimizerModel import GDOptimizer
from pbc.model.TrainingModel import MaximumBayesBoundarynessTraining
from pbc.trainer.MBBTrainer import MBBTrainer
...
def main():
    ...
    if config.name == "":
        config.name = "%s-%s" % (os.path.basename(SCRIPT_NAME), os.path.basename(config.dataset).split(".")[0])
    ...
    cy_train, N_cy_train = np.unique(np.argmax(y_train, axis=1), return_counts=True)
    ...
    classifier_model = MultiClassKmeansPrototypeClassifier(config, x_train, y_train)
