...
OUTPUT_DIR = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../../dataset/HO"))
...
def load_data(dataset_path, delimiter=',', training=0.5, testing=0.5, output_dir=OUTPUT_DIR,
    ...
    if not os.path.exists(os.path.dirname(dataset_path)):
        print (" *** load_data: ERROR; Cannot find dataaset dir.")
    ...
    save_path = os.path.join(output_dir, "%s_training-%s_testing-%s.csv" % (base_name, trn, tsn))
    ...
    if load_split_data:
        if overwrite:
            ...
            if not os.path.exists(save_path):
                print (" *** WARNING: %s could not find in the path, Create new dataset" % save_path)
            else:
                # Load stored data
                print ("  * Load stored data from %s" % save_path)
                exist_data = np.genfromtxt(save_path, delimiter=delimiter, filling_values=0)
                x_data = exist_data[:, :-1].astype(float)
                y_class = exist_data[:, -1].astype(int)
                if make_one_hot_label:
                    one_hot = np.identity(max(y_class) + 1)[y_class]  # One-hot vector (max_value + 1 (for "0" class))
    ...
    if randomize:
        ...
        for yc in y_cat:
            ...
            tr_idx = np.random.choice(yc_idx, size=int(len(yc_idx) * training_sample_ratio), replace=False)
    ...
    if save_split_data:
        messages = []
        messages.append(" * Create date: %s" % datetime.datetime.today().strftime("%Y-%m-%d_%H:%M:%S"))
        ...
        for i in range(len(y)):
            ...
            messages.append("   - Class %s: %s (class ratio: %s)" % (y[i], N_y[i], R_y))
        y_tr, N_y_tr = np.unique(y_class[tri], return_counts=True)
        S_y_tr = np.sum(N_y_tr) / len(y_class)
        messages.append("  - Training Samples: %s (sample ratio: %s)" % (np.sum(N_y_tr), S_y_tr))
        for i in range(len(y_tr)):
            R_y_tr = N_y_tr[i] / np.sum(N_y_tr)
            messages.append("   - Class %s: %s (class ratio: %s)" % (y_tr[i], N_y_tr[i], R_y_tr))
        y_ts, N_y_ts = np.unique(y_class[tsi], return_counts=True)
        S_y_ts = np.sum(N_y_ts) / len(y_class)
        messages.append("  - Testing Samples: %s (sample ratio: %s)" % (np.sum(N_y_ts), S_y_ts))
        for i in range(len(y_ts)):
            R_y_ts = N_y_ts[i] / np.sum(N_y_ts)
            messages.append("   - Class %s: %s (class ratio: %s)" % (y_ts[i], N_y_ts[i], R_y_ts))
        train_data = np.hstack((x_data[tri], np.reshape(y_class[tri], (-1, 1))))
        ...
        output_txt(os.path.join(output_dir, "log_%s_training-%s_testing-%s.txt" % (base_name, trn, tsn)), messages)
