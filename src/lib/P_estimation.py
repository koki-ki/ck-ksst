...
def estimate_P_on_B_segmentation(sess, classifier, B, Xtr, ytr, dy, seg_size=40, multiprocess=True):
    ...
    B_seg_ind, g_seg_ind = [], []
    B_seg = np.array([np.argsort([np.sum(np.square(br - x)) for x in Xtr])[:seg_size] for br in B])  # B * seg
    for Bsi in B_seg:
        ...
        if np.count_nonzero(OB_gs_ind_oh * OB_yt) == 0:
            continue
        OB_gs_ind = sess.run(classifier.gs_ind,
                             feed_dict={classifier.x: [Xtr[OB_ind]], classifier.y: [ytr[OB_ind]]})[0]
    ...
    seg_args = [(S, Bsi, gsi, dy[Bsi], yt) for Bsi, gsi in zip(B_seg_ind, g_seg_ind)]
